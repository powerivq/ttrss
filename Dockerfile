...
RUN apk add --update --no-cache --virtual .build-deps curl-dev icu-dev libxml2-dev libressl-dev pcre-dev \
    && apk add --update --no-cache bzip2-dev freetype-dev gettext-dev libjpeg-turbo-dev libpng-dev supervisor \
    && docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include \
    && docker-php-ext-install bz2 curl dom gd gettext mbstring opcache iconv mysqli opcache pcntl pdo pdo_mysql phar posix xml xmlrpc \
